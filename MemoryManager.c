 ...
 *
 y      2.e Check if the median is divisible by 13.
 *
 *      2.f Return the number of medians that were divisible by 13.
 *
 */
int get_running_count(){
// 1. Generating the number of interations
...
// 2. For each iteration
	printf("[MemoryManager] Number of iterations: %d\n", iter);	
 	for(int j = 0; j < iter; j++){
		...
		int *arrPointer = (int *)malloc(sizeof(int) * arrSize);
	
		for(int i = 0; i < arrSize; i++){
			arrPointer[i] = rand();
		}

		int median = return_median(arrPointer, arrSize);
